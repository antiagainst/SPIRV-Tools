...
bool DeadBranchElimPass::MarkLiveBlocks(
    Function* func, absl::flat_hash_set<BasicBlock*>* live_blocks) {
  StructuredCFGAnalysis* cfgAnalysis = context()->GetStructuredCFGAnalysis();

  absl::flat_hash_set<BasicBlock*> continues;
...
bool DeadBranchElimPass::FixPhiNodesInLiveBlocks(
    ...
    const absl::flat_hash_map<BasicBlock*, BasicBlock*>& unreachable_continues) {
  bool modified = false;
  for (auto& block : *func) {

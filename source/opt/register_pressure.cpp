 ...
 private:
  ...
  void EvaluateRegisterRequirements() {
    for (BasicBlock& bb : *function_) {
      ...
      absl::flat_hash_set<uint32_t> die_in_block;
      for (Instruction& insn : make_range(bb.rbegin(), bb.rend())) {
...
void RegisterLiveness::ComputeLoopRegisterPressure(
  ...
  absl::flat_hash_set<uint32_t> exit_blocks;
  ...
  absl::flat_hash_set<uint32_t> seen_insn;
...
void RegisterLiveness::SimulateFission(
    const Loop& loop, const absl::flat_hash_set<Instruction*>& moved_inst,
    const absl::flat_hash_set<Instruction*>& copied_inst,

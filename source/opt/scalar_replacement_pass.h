...
class ScalarReplacementPass : public Pass {
 ...
 private:
  ...
  // is possibly used.
  std::unique_ptr<absl::flat_hash_set<uint64_t>> GetUsedComponents(
      Instruction* inst);
  ...
  absl::flat_hash_map<uint32_t, uint32_t> pointee_to_pointer_;

  // Maps type id to OpConstantNull for that type.
  absl::flat_hash_map<uint32_t, uint32_t> type_to_null_;

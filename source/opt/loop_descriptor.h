...
class Loop {
 ...
 public:
  ...
  using BasicBlockListTy = absl::flat_hash_set<uint32_t>;
  ...
  void GetExitBlocks(absl::flat_hash_set<uint32_t>* exit_blocks) const;
  ...
  // unreachable.
  void GetMergingBlocks(absl::flat_hash_set<uint32_t>* merging_blocks) const;
...
class LoopDescriptor {
 ...
 private:
  ...
  inline Loop* FindLoopForBasicBlock(uint32_t block_id) const {
    absl::flat_hash_map<uint32_t, Loop*>::const_iterator it =
  ...
  absl::flat_hash_map<uint32_t, Loop*> basic_block_to_loop_;
